...
import { Activity, Cable, CheckCircle2, Database, Loader2, RefreshCw, Send, ShieldCheck, TriangleAlert } from 'lucide-react';
...
export function UtmifyGlobalCenter() {
  ...
  const save = useMutation({
    mutationFn: () => apiClient.saveUtmifyGlobal({ name: form.name.trim(), ...(form.api_token.trim() ? { api_token: form.api_token.trim() } : {}), endpoint_url: form.endpoint_url.trim(), pixel_id: form.pixel_id.trim(), enabled: form.enabled }),
  ...
  return <div className="signal-reveal space-y-6">
    ...
    <section className="rounded-2xl border border-cyan-300/15 bg-[#071720]/90 p-5 sm:p-6">
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2">
        ...
        <input value={form.pixel_id} onChange={(event) => setForm({ ...form, pixel_id: event.target.value })} placeholder="Pixel ID geral (24 caracteres)" className="h-11 rounded-lg border border-white/10 bg-black/20 px-4 font-mono text-sm text-white outline-none focus:border-cyan-300/40" />
      ...
      <div className="mt-4 flex flex-wrap justify-end gap-2"><Button variant="outline" onClick={() => test.mutate()} disabled={!config.data?.configured || test.isPending} className="gap-2 border-white/10"><Send className="h-4 w-4" /> Enviar pedido teste</Button><Button onClick={() => save.mutate()} disabled={save.isPending || !form.name.trim() || !/^[a-f0-9]{24}$/i.test(form.pixel_id.trim()) || (!form.api_token.trim() && !config.data?.destination?.token_configured)} className="bg-cyan-300 text-slate-950 hover:bg-cyan-200">{save.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Salvar UTMify Geral</Button></div>
    </section>
