    ...
    {report.isLoading ? <div className="flex items-center gap-2 p-10 text-white/50"><Loader2 className="h-4 w-4 animate-spin"/>Carregando dados financeiros…</div> : <>
      ...
      <section className="grid gap-5 xl:grid-cols-[1.45fr_1fr]">
        <div className="rounded-2xl border border-white/[.09] bg-[#071720]/70 p-5"><div className="flex items-center justify-between gap-3"><div><p className="hud-label">Evolução</p><h2 className="mt-1 text-lg font-semibold text-white">Valor revertido por dia</h2></div><CreditCard className="h-5 w-5 text-cyan-200/70" /></div>
          {!data?.daily.length ? <Empty /> : <div className="mt-7 flex h-52 items-end gap-1.5">{data.daily.map(day=><div key={day.date} className="group flex h-full min-w-5 flex-1 flex-col justify-end"><div title={`${day.date}: ${money(day.brl_minor)}`} className="min-h-1 rounded-t bg-gradient-to-t from-rose-500/70 to-amber-300/80 transition group-hover:brightness-125" style={{height:`${Math.max(3,(day.brl_minor/maxDaily)*100)}%`}} /><span className="mt-2 -rotate-45 origin-top-left whitespace-nowrap font-mono text-[8px] text-white/35">{day.date.slice(5)}</span></div>)}</div>}
        </div>
