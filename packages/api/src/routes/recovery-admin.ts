...
const plugin: FastifyPluginAsync = async (app: FastifyInstance) => {
  ...
  app.get<{ Params: { id: string } }>('/offers/:id/recovery', async (req, reply) => {
    ...
    const [sources] = await app.db<Array<{ gateway: string | null; gateway_enabled: boolean; ab_test: string | null; ab_destinations: number; entry_links: number; entry_clicks: number; vendepay_webhooks: number }>>`
      SELECT
    ...
    `;
    return { settings: settings ?? null, sources: { ...sources, gateway: sources?.gateway ?? (sources?.gateway_enabled ? 'vendepay' : null), automatic: Boolean(sources?.gateway_enabled && ((sources?.ab_destinations ?? 0) > 0 || settings?.checkout_url)) }, channels: channels.map((c) => ({ ...c, configured: true })), totals: totals[0], opportunities: opportunities.map((o) => ({ ...o, email: mask(o.email as string | null, 'email'), phone: mask(o.phone as string | null, 'phone'), has_email: Boolean(o.email), has_phone: Boolean(o.phone) })) };
  });
  ...
  app.post<{ Params: { id: string } }>('/offers/:id/recovery/sync', async (req, reply) => {
    ...
    for (const order of candidates) { if (!order.destination_url) { skipped++; continue; } let destination:string; try { destination=appendAttribution(order.destination_url, order.attribution_source); } catch { skipped++; continue; } const token=randomBytes(24).toString('base64url'); const rows=await app.db`
